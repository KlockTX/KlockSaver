...
process.stdin.on('end', () => {
  ...
  const deny = (why) => {
    ...
    console.error('[token-saver guard] ' + why);
    process.exit(2);
  };
  ...
  if (/^Bash$/i.test(name)) {
    ...
    if (/\b(cat|head\s+-n?\s*9{3,}|less|more)\b.*\.(log|jsonl?|csv|txt)\b/i.test(cmd) && !/\||head/.test(cmd))
      return deny(`Reading whole log/data file into context. Pipe through: | node "$HOME/.qoder-cn/skills/token-saver/scripts/tok_reduce.cjs" or grep the error lines only.`);
    ...
    if (/\b(npm|yarn|pnpm)\b.*(install|build|test)\b/.test(cmd) && !/\|.*(tail|grep|tok_reduce)/.test(cmd) && /--?(verbose|loglevel\s*=)/.test(cmd))
      return deny(`Verbose package-manager output into context. Drop --verbose or pipe: 2>&1 | node ~/.qoder-cn/skills/token-saver/scripts/tok_reduce.cjs`);
  }
  if (/^Read$/i.test(name)) {
